...
class BillOrderSerializer(serializers.Serializer):
    ...
    buyerGstn = serializers.CharField()
    ...
    vendorGstn = serializers.CharField()
    vendorUniqueId = serializers.CharField()
    vendorGstStatus = serializers.CharField()
    ...
    def create(self, validated_data):
        consignee_data = validated_data.pop('consigneeDetails')
        consignment_details = []
        for consignee in consignee_data:
            ...
            for item in items_data:
                ...
                for billing_cycle_detail in billing_cycle_detail_data:
                    billing_cycle, created = BillBillingCycleDetail.objects.get_or_create(**billing_cycle_detail)
                    billing_cycle_details.append(billing_cycle)
                product, created = BillProduct.objects.get_or_create(**item)
                product.billing_cycle_details.set(billing_cycle_details)
                items.append(product)
...
class BillSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        for order in order_data:
            consignee_data = order.pop('consigneeDetails')
            consignment_details = []
            for consignee in consignee_data:
                items_data = consignee.pop('items')
                items = []
